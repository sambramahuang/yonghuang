...
import { fixtureExtractor, liveExtractor, fixtureDrafter, liveDrafter, fixtureDiscoverer, liveDiscoverer, fixtureRegulatoryExtractor, liveRegulatoryExtractor } from './extract/providers.js';
...
const regulatoryExtractor = mode === 'live' ? liveRegulatoryExtractor({ apiKey: process.env.OPENAI_API_KEY,model: process.env.OPENAI_MODEL }) : fixtureRegulatoryExtractor();
const app = createApp({ pool,secret: process.env.AUTH_SECRET,extractor,drafter,discoverer,regulatoryExtractor,extractionMode: mode,corsOrigin: process.env.CORS_ORIGIN });
